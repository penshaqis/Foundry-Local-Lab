...
HOW TO MODIFY CHUNKING IN PYTHON:
...
>#### ???? CM QUESTION: Does this means if you have a GPU, 1. you can safely run RETRIEVAL using embedding vector model on the CPU only and 2. you can run the LLM on the NPU for generation (via the user prompt)???? 
