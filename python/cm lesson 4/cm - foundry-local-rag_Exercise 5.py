...
# Knowledge base is the collection of all individual chunks, or files.
# Chunks range from complex unstructured files or simple "Title & Content" strings, like below.

# For learning purposes, we begin this first Retrival exercise with sample chunks of Title & Content strings.

# 1. Retrieval: Deciding which of the knowledge base's chunks are most relevant to USER'S QUERY/PROMPT.
    # Retrieval functions are plenty, however, again for easy understating, we'll use "keyword overlap".

# 2. Next the "SELECTED" chunks - those with the best keyword overlap - are placed into the context window
# 3. After the context window is up-to-date, the LLM can then proceed to generate answer, per usual.
